...
app.get('/verifywebreport/:code', function(req, res) {
	dispatcher.verifyWebreport(req.params.code, function (message) {
        res.send('<html><head><title>Verification</title><style>*{margin:0px;padding:0px;}</style></head><body><h1 align="center"><img src="https://upload.wikimedia.org/wikipedia/commons/4/48/Pace_University_logo.png" width="150px" alt="Pace University"></h1><br><br><p style="text-align:center;border:5px solid black;margin-left:20%;width:60%;padding:5%;">' + message + '</p></body></html>');
    });
});
